...
unsigned char sprawdz_wyswietlanie_opcji_po_enter(unsigned char nr, unsigned char which_marked);
...
typedef struct   linia_menu{
	...
	unsigned int  variables_table[3];
...
#define Wymiar_Menu   11
//inna postac struktury
Menu_t  MyMenu[] = {
	{(unsigned char *)"mode set", 1 , {3,0} },
	{(void *)"display mode", 1, {0,0}},
	{(void *)"bajt", 1, {4,5 } },
	{(void *)"odczyt bajtu", 2, {0,1}},
	{(void *)"linia",1,{0} },
	{(void *)"point",3, {2,0,1} },
	// {(void *)"timer",1, {3}, &menu_write_option_value },
	{(void *)"timer",1, {3}},
	{(void *)"fill screen",1,{3} },
...
void main(void){
	...
	__delay_ms(1000);
	clear_screen(0x00, t6963_columns * t6963_rows*8, 0x0c00 );
	__delay_ms(1000);
	...
	while(1){
		...
		if ((jak_dlugo_wcisniete> jak_dlugo_klawisz_do_zmiany) && (znaczniki & czy_jestem_w_opcji) && ((KEY_RED_PORT == PRESSED) || (KEY_BLUE_PORT == PRESSED)) ){
				...
				zmiana_opcji(nr_opcji, &(MyMenu[nr_opcji].variables_table[nr_marked_value]));
				// sprawdz_wyswietlanie_opcji_po_enter(nr_opcji, nr_marked_value);
				menu_write_option_value( nr_opcji,  nr_marked_value);
		...
		if (KEY_BLACK_PORT == PRESSED && PIE1bits.TMR1IE == 0 ) {
			...
			if ( (znaczniki & czy_jestem_w_opcji) && ( (MyMenu[nr_opcji].how_many_options) == 0) ) {
			...
			else if ( (znaczniki & czy_jestem_w_opcji) && ( nr_marked_value == (MyMenu[nr_opcji].how_many_options-1) ) ) {
				//odznaczam
				...
				if (MyMenu[nr_opcji].function)
					MyMenu[nr_opcji].function();
				else
				{
					znaczniki |= czy_jestem_w_opcji;
					// sprawdz_wyswietlanie_opcji_po_enter(nr_opcji, nr_marked_value);
...
void  show_menu(unsigned char nr_zaznaczony, unsigned char stan){
	...
	for(pos_start=0; MyMenu[pos_start].napis ; pos_start++){
		...
		if ( (nr_zaznaczony == pos_start) && (stan !=0) ){
			...
			ustaw_atrybut_tekstu(0,pos_start, MyMenu[pos_start].napis, 0);	
...
void   menu_write_option_value(unsigned char nr_line, unsigned char which_marked){
	unsigned char  i , string1[5];
		...
		for( i=0; i<MyMenu[nr_line].how_many_options ; i++){
			...
			if (i == which_marked )
				ustaw_atrybut_tekstu(pos_option_menu+i*3,nr_line, string1, atrybut_zmiany_var);
			else	
				ustaw_atrybut_tekstu(pos_option_menu+i*3,nr_line, string1, 0);
